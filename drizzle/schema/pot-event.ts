...
CREATE TABLE "pot_event" (
    ...
    "id"            smallint                 NOT NULL DEFAULT nextval('pot_event_id'::regclass),
    "type"          pot_event_type           NOT NULL,
    "data"          jsonb                    NOT NULL,
    PRIMARY KEY ("pot_fk", "timestamp", "id")
);
CREATE INDEX "idx_pot_event_n_pot_fk_type" ON "pot_event" ("pot_fk", "type");
...
export const potEvent = pgTable(
  "pot_event",
  {
    ...
    id: smallint("id")
      .default(sql`nextval('pot_event_id'::regclass)`)
      .notNull(),
  ...
  (table) => [
    primaryKey({
      name: "pot_event_pkey",
      columns: [table.potFk, table.timestamp, table.id],
    }),
    index().on(table.potFk),
    index().on(table.potFk, table.type),
  ],
);
